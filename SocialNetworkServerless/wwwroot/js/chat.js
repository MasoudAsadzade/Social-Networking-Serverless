$(document).ready(function () {
    var connection = new signalR.HubConnectionBuilder().withUrl("/timelineHub").build();
    ...
    function AppViewModel() {
        ...
        self.sendNewMessage = function () {
            ...
            if (text.startsWith("/")) {
                ...
                if (receiver.length > 0 && message.length > 0)
                    connection.invoke("SendtoPersonalTimeline", self.joinedRoom(), receiver.trim(), message.trim());
            }
            else {
                if (self.joinedRoom().length > 0 && self.message().length > 0)
                    connection.invoke("SendtoPublicTimeline", self.joinedRoom(), self.message());
            }
        ...
        self.messageHistory = function () {
            connection.invoke("GetTimelineHistory", self.joinedRoom()).then(function (result) {
        ...
        self.uploadFiles = function UserViewModel() {
            var self = this;
            ...
            self.GetUsers = function () {
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: baseUrl + 'api/Timeline/SendtoPersonalTimeline',
                    data: jason.strigfy(),
